    ...
    public function __invoke($_, array $args)
    {
        ...
        if ($actual_month != 1 && $actual_month != 2) {
            ...
            $ministred_actual_query =
                DB::table('threemonths_concepts_curriculum_view')->select('ministered_lives')->where([
                    ['year', $year],
                    ['month', $actual_month],
        ...
        $members_query = DB::table('threemonths_concepts_curriculum_view')->select('current_membership')->where([
            ['year', $year],
            ['month', $actual_month],
            ['curriculum_id', $curriculum_id]
        ])->get();
        ...
        //CADIDATOS AL BAUTISMO
        $candidates_query =
            DB::table('threemonths_concepts_curriculum_view')->select('baptism_candidates')->where([
                ['year', $year],
                ['month', $actual_month],
                ['curriculum_id', $curriculum_id]
            ])->get();
        ...
        //NUEVOS CREYENTES
        $news_query =
            DB::table('threemonths_concepts_curriculum_view')->select('new_believers_discipled')->where([
                ['year', $year],
                ['month', $actual_month],
                ['curriculum_id', $curriculum_id]
            ])->get();
        ...
        $kids_query
            = DB::table('threemonths_departments_curriculum_view')->select('s1', 's2', 's3', 's4')->where([
                ['year', $year],
                ['month', $actual_month],
                ['curriculum_id', $curriculum_id],
                ['department_id', 5]
