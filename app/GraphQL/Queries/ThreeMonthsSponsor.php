    ...
    public function __invoke($_, array $args)
    {
        ...
        if ($actual_month != 1 && $actual_month != 2) {
            ...
            $ministred_actual_query =
                DB::table('threemonths_concepts_sponsor_view')->select('ministered_lives')->where([
                    ['year', $year],
                    ['month', $actual_month],
        ...
        //MEMBRESIA ACTUAL
        $members_query = DB::table('threemonths_concepts_sponsor_view')->select('current_membership')->where([
            ['year', $year],
            ['month', $actual_month],
            ['sponsor_id', $sponsor_id]
        ])->get();
        ...
        //DICIPULADO
        $candidates_query =
            DB::table('threemonths_concepts_sponsor_view')->select('baptism_candidates')->where([
                ['year', $year],
                ['month', $actual_month],
                ['sponsor_id', $sponsor_id]
            ])->get();
        ...
        //NUEVOS CREYENTES
        $news_query =
            DB::table('threemonths_concepts_sponsor_view')->select('new_believers_discipled')->where([
                ['year', $year],
                ['month', $actual_month],
                ['sponsor_id', $sponsor_id]
            ])->get();
        ...
        $kids_query
            = DB::table('threemonths_department_sponsor_view')->select('s1', 's2', 's3', 's4')->where([
                ['year', $year],
                ['month', $actual_month],
                ['sponsor_id', $sponsor_id],
                ['department_id', 5]
